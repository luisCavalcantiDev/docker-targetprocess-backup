...
@test "backup features works" {
  ...
  refute_output --partial "Errors from the request:  null"
  refute_output --partial "Errors from the request"
...
@test "backup using invalid.js fails" {
  run docker run --name ${cont} -ti \
    --env TEST=true \
    --env TP_DOMAIN="${TP_DOMAIN}" \
    --env TP_USER="${TP_USER}" \
    ...
    -v ${volume_data}:/tmp/tp_backup \
    "${this_image_name}:${this_image_tag}" \
    "source /opt/tp_backup/run_functions.sh && mkdir -p \$BACKUP_DIR && verify_credentials && nodejs ./entities/invalid.js 2700 2800 > \$BACKUP_DIR/test.json"
  assert_output --partial "Credentials verified"
  refute_output --partial "not set, please set it"
  refute_output --partial "Errors from the request:  null"
  assert_output --partial "Errors from the request"
  assert_output --partial "\"Status\":\"BadRequest\""
  assert_output --partial "Error during parameters parsing"
  assert_equal "$status" 0
